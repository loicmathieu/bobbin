...
import com.pironet.tda.HeapInfo;
...
import com.pironet.tda.parser.AbstractDumpParser;
...
public class SunJDKParser extends AbstractDumpParser {
	...
	public MutableTreeNode parseNext() {
		...
		do {
			...
			try {
				...
				Stack<String> monitorStack = new Stack<>();
				long startTime = 0;
					...
					if (locked) {
						...
						if (line.startsWith("\"")) {
							...
							if(title.contains("in ")){
								...
								try {
									//FIXME
									//ugly but needed to be able to parse multiple format
							...
							//case the thread state is not in the thread title
							content.append(line);
							content.append("\n");
	...
	private int[] dumpBlockingMonitors(DefaultMutableTreeNode catLockingTree, MonitorMap mmap) {
		Map<String, DefaultMutableTreeNode> directChildMap = new HashMap<String, DefaultMutableTreeNode>(); // Top level of our display model
		...
		//********************************************************************
		for (Object element : directChildMap.entrySet()) {
			DefaultMutableTreeNode threadNode = (DefaultMutableTreeNode) ((Map.Entry) element).getValue();
	...
	private void renormalizeBlockingThreadTree(MonitorMap mmap, Map<String, DefaultMutableTreeNode> directChildMap) {
		Map<String, DefaultMutableTreeNode> allBlockingThreadsMap = new HashMap<String, DefaultMutableTreeNode>(directChildMap); // All threads that are blocking at least one other thread
		...
		for (Iterator iter = mmap.iterOfKeys(); iter.hasNext();) {
			...
			DefaultMutableTreeNode thread1Node = allBlockingThreadsMap.get(monitor1);
			if (thread1Node == null) {
				continue;
			}
			...
			for (Iterator iter2 = mmap.iterOfKeys(); iter2.hasNext();) {
				...
				if (threads2[MonitorMap.WAIT_THREAD_POS].containsKey(threadLine1)) {
					...
					DefaultMutableTreeNode thread2Node = allBlockingThreadsMap.get(monitor2);
		...
		do {
			changed = false;
			for (Object element : directChildMap.entrySet()) {
				DefaultMutableTreeNode node = (DefaultMutableTreeNode) ((Map.Entry) element).getValue();
		...
		for (Object element : directChildMap.entrySet()) {
			renormalizeThreadDepth((DefaultMutableTreeNode) ((Map.Entry) element).getValue());
		}
	}
	...
	private boolean checkForDuplicateThreadItem(Map<String, DefaultMutableTreeNode> directChildMap, DefaultMutableTreeNode node1) {
	...
	public void parseLoggcFile(InputStream loggcFileStream, DefaultMutableTreeNode root) {
		...
		Vector<HistogramTableModel> histograms = new Vector<HistogramTableModel>();
